...
import { FaStore, FaSignOutAlt, FaRedo, FaCheck, FaTimes, FaTruck, FaUser, FaClock, FaMapMarkerAlt } from 'react-icons/fa';
...
const DeliveryVendorDashboard: React.FC = () => {
  ...
  useEffect(() => {
    console.log('🏪 DeliveryVendorDashboard useEffect:', {
      authLoading,
      vendorFromContext,
    ...
    if (!authLoading) {
      if (!vendorFromContext) {
        ...
        console.log('✅ Loading delivery orders for vendor:', vendorFromContext.id);
        loadDeliveryOrders(vendorFromContext.id);
      }
    }
  ...
  const loadDeliveryOrders = async (vendorId: string) => {
    ...
    if (!vendorId || typeof vendorId !== 'string' || vendorId === 'undefined' || vendorId === '[object Object]') {
      console.error('❌ Invalid vendor ID received in DeliveryVendorDashboard:', vendorId, 'Type:', typeof vendorId);
      toast.error('Invalid vendor session. Please log in again.');
      return;
    }

    console.log('🔍 Loading delivery orders for vendor ID:', vendorId);
    ...
    try {
      ...
      console.log('✅ Loaded delivery orders:', ordersData?.length || 0);
  ...
  const handleCancelOrder = async (orderId: string) => {
    try {
      const result = await orderService.cancelOrder(orderId, 'Cancelled by delivery vendor');
      if (result.success) {
        setOrders(prev => prev.map(order =>
    ...
    <Container>
        ...
        <HeaderContent>
          <VendorInfo>
            ...
            <VendorDetails>
              ...
              <VendorSubtitle>Delivery Orders Dashboard</VendorSubtitle>
            </VendorDetails>
          </VendorInfo>
          <HeaderActions>
            <ActionButton onClick={() => vendor && loadDeliveryOrders(vendor.id)}>
              <FaRedo />
            </ActionButton>
      ...
      {loading ? (
        <EmptyState>
          ...
          <h3>Loading delivery orders...</h3>
        </EmptyState>
      ) : orders.length === 0 ? (
        <EmptyState>
          <EmptyStateIcon><FaTruck /></EmptyStateIcon>
        ...
        orders.map((order) => (
          ...
          >
            <OrderHeader>
              <OrderId>Delivery Order #{order.bill_number || order.id}</OrderId>
              ...
              <DetailRow>
                ...
                <DetailValue>
                  {order.delivery_address ? 
                    `${order.delivery_address.addressLine1}, ${order.delivery_address.addressLine2 || ''}` : 
...
export default DeliveryVendorDashboard;
